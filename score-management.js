...
class ScoreManagement {
    constructor() {
        this.apiBaseUrl = 'http://localhost:3000/api/scoreManagement';
        this.currentPage = 1;
        this.pageSize = 10;
        this.totalPages = 1;
        this.sports = [];
    ...
    async loadMatches() {
        try {
            ...
            if (statusFilter) params.append('is_played', statusFilter);
    ...
    createMatchCard(match) {
        const statusClass = match.rain_canceled ? 'status-canceled' : 
                           match.is_played ? 'status-played' : 'status-not-played';
        const statusText = match.rain_canceled ? '우천취소' : 
                          match.is_played ? '완료' : '미완료';

        const homeTeam = match.home_team || { name: 'TBD', score: 0 };
        const awayTeam = match.away_team || { name: 'TBD', score: 0 };
            ...
            <div class="match-card" data-match-id="${match.id}">
                <div class="match-header">
                    <div class="match-info">
                        ...
                        <div class="match-details">${match.sport} | ${match.venue}</div>
                    </div>
                ...
                <div class="match-actions">
                    <button class="btn btn-primary" onclick="scoreManagement.openScoreModal(${match.id})">
                        점수 수정
                    </button>
                    <button class="btn btn-secondary" onclick="scoreManagement.openStatusModal(${match.id})">
                        상태 수정
                    </button>
                </div>
    ...
    async openScoreModal(matchId) {
        try {
            ...
            document.getElementById('modalHomeScore').value = match.home_team?.score || 0;
            document.getElementById('modalAwayScore').value = match.away_team?.score || 0;
            document.getElementById('modalIsPlayed').checked = match.is_played;
            document.getElementById('modalAdminNote').value = match.admin_note || '';
    ...
    async openStatusModal(matchId) {
        try {
            ...
            document.getElementById('statusModalIsPlayed').checked = match.is_played;
            document.getElementById('statusModalRainCanceled').checked = match.rain_canceled;
            document.getElementById('statusModalAdminNote').value = match.admin_note || '';
    ...
    async updateMatchScores() {
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/matches/${matchId}/scores`, {
                method: 'PUT',
                headers: {
